  ...
  const questions = chapter.questions
    .map(
      (question, questionIndex) =>
        `<details><summary>${questionIndex + 1}. ${inline(question.question)} <small>(${esc(question.level)}${question.topic ? ` · ${esc(question.topic)}` : ''})</small></summary><div class="answer">${question.answer.map((answer) => `<p>${inline(answer)}</p>`).join('')}${question.example ? `<pre>${esc(question.example)}</pre>` : ''}${question.fixedFormat ? `<h4>${esc(question.fixedLabel || 'Fixed-format RPG')}</h4><pre>${esc(question.fixedFormat)}</pre>` : ''}${question.freeFormat ? `<h4>${esc(question.freeLabel || 'Fully free RPG')}</h4><pre>${esc(question.freeFormat)}</pre>` : ''}${question.trap ? `<p><strong>Interview pitfall:</strong> ${inline(question.trap)}</p>` : ''}</div></details>`,
    )
    .join('');
