...
export type ReferenceData = { sqlModules: SqlModule[]; fileOps: FileOp[]; comparisons: Comparison[]; errors: ErrorItem[]; sources: Source[] };
...
export function ReferenceHub({ data, tab = 'sql' }: { data: ReferenceData; tab?: string }) {
  ...
  return <article className="reference-hub">
    ...
    <nav className="reference-tabs" aria-label="SQL and file operation guides">
      {[['sql', 'Db2 for i course'], ['files', 'RPG file opcodes'], ['compare', 'RPG ↔ SQL comparison'], ['errors', 'Error handling']].map(([id, label]) => <button key={id} className={active === id ? 'chosen' : ''} aria-pressed={active === id} onClick={() => change(id)}>{label}</button>)}
    </nav>
